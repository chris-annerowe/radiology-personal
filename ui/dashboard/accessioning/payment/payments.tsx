...
export default function Payments(props:PaymentProps) {
    ...
    const handleProviderChange = (e) => {
        ...
        // Send the selected value immediately as a prop
    ...
    return (
        ...
        <div>
            ...
            {clientProviders.map((prov, index) => (
                <option value={prov.clientprov_name} id={`clientProvider-${index}`}>
                {prov.clientprov_desc}
                </option>
            ))}
            </Select>
        </div>

        {/* Conditionally render Method field */}
        {selectedProvider === '' && (
            <div>
            <div className="mb-2 block">
            ...
            <Select id="method" name="method" defaultValue="" sizing="sm" disabled={false}>
                ...
                <option value={type.abbreviation} id={`paymentType-${index}`}>
                    {type.name}
                </option>
                ))}
            </Select>
        ...
        <div className="flex my-2 justify-end">
            <Button className="w-10" type="submit" color="grey">
            <HiPlus />
            </Button>
        </div>
        </form>
